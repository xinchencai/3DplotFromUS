...
for i = 1:len
    US(i).myus = imread(['RotatingScan/' num2str(i) '.jpg']);
end
...
%         n = 1;
%         P = P1'
% %         [P(1,:),P(3,:),P(2,:)] = pol2cart(P1(:,3),P1(:,1),P1(:,2));
%     elseif i == 2
%         n = 2;
%         P = P2'
% %         [P(1,:),P(3,:),P(2,:)] = pol2cart(P2(:,3),P2(:,1),P2(:,2));
%     elseif i == 3
%         n = 4;
%         P = P3'
% %         [P(1,:),P(3,:),P(2,:)] = pol2cart(P3(:,3),P3(:,1),P3(:,2));
%     elseif i == 4
%         n = 9;
%         P = P4'
% %         [P(1,:),P(3,:),P(2,:)] = pol2cart(P4(:,3),P4(:,1),P4(:,2));
%     end
    [P(1,:),P(2,:),P(3,:)] = pol2cart(Px(i).myP(:,3),Px(i).myP(:,1),Px(i).myP(:,2));
...
figure;
hold on
plot(t,error_alpha,'m','DisplayName','Alphashape');
% plot(t,error_area,'y','DisplayName','Sum of area');
plot(t,error_bul,'c','DisplayName','Bullet formula');
plot(t,error_mvee,'g','DisplayName','MVEE');
plot(t,error_tri,'b','DisplayName','Trisurf');
plot(t,zeros(9,1),'r','DisplayName','MRI')
hold off
xlabel('Every nth slice taken')
% xlabel('Number of slices taken')
